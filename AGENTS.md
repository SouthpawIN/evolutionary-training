...
1. **Stage 1 — Agentic SFT** (complete Jun 11 2026): QLoRA on Qwen3-8B base with
...
**Stage 1 is COMPLETE (Jun 11, 2026).** Awaiting LoRA merge and HF upload.

| Metric | Value |
|---|---|
| Final step | 3954 / 3954 |
| Runtime | 20h 19m |
| Final loss | 0.2352 |
| Best loss | 0.2216 @ epoch 1.62 |
| Token accuracy | 92.27% |
| LoRA adapter | 667 MB (`adapter_model.safetensors`) |
| Output dir | `training-output/omnisenter-sft-20260606_213858/` (3.7 GB) |

GPUs: both RTX 3090s are back in use by `llama-darwin` and `llama-apex`.

## What you should NOT do

1. **Do not touch Stage 1 output.** It's in `training-output/omnisenter-sft-20260606_213858/`. Don't modify the training script or change the data path. The LoRA adapter is the input for the next stage.
...
1. **After Stage 1 complete, run `merge_lora.py`** to merge LoRA weights into base, then `yarn_256k_config.py` for long-context pass.
2. **Check training status** with `ps -p <PID> -o etime,pcpu,stat`
3. **Read the training log** at `logs/training_v3_<date>.log`
4. **Tail the trainer state** at `training-output/<run>/checkpoint-*/trainer_state.json`
5. **Use `--resume`** if restarting the training (auto-detects latest checkpoint)
6. **Validate the catalog** before pushing: `python3 -c "import yaml; yaml.safe_load(open('models/curated.yaml'))"`

## TurboFit note (2026-06-22)

Local model server infrastructure is TurboFit v5. `serve auto main` / `serve downscale`. Do NOT use raw llama-server or old llama-launch commands.
